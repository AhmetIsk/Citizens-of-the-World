...
/**
 * This class is created to read the text files. 
 * It includes many static methods that are called from PlayerMenu, MusicPlayer and Game classes
...
{  
   ...
   // @return names of countries alpahabetically
   ...
   {     
      ...
      final int FLAG_QUESTION_ROW = 3; // in the text file, flag questions are placed as 3. questions.
      ...
      try
      {
         ...
         while ( countryCount < COUNTRY_NUMBER )
         {
            while ( questionCount < QUESTION_NUMBER )
            {
               ...
               if ( infoCount == 0 )
               {
                  questions.add( new Question( questionSentence, choices, answer, questionCount == FLAG_QUESTION_ROW - 1 ) );
                  questionCount++;
